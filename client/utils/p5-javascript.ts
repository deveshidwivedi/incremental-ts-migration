...
(function (mod: any) {
  ...
  // Plain browser env
  else mod((window as any).CodeMirror);
})(function (CodeMirror: typeof CodeMirrorNS) {
  'use strict';

  CodeMirror.defineMode(
    'javascript',
    function (config: any, parserConfig: any) {
      ...
      const keywords = (function () {
        ...
        const jsKeywords: Record<string, any> = {
          if: kw('if'),
          while: A,
          with: A,
          else: B,
        ...
        for (const attr in p5FunctionKeywords) {
          jsKeywords[attr] = (p5FunctionKeywords as any)[attr];
        }

        for (const attr in p5VariableKeywords) {
          jsKeywords[attr] = (p5VariableKeywords as any)[attr];
        }

        // Extend the 'normal' keywords with the TypeScript language extensions
        if (isTS) {
          const type = { type: 'variable', style: 'variable-3' };
          const tsKeywords: Record<string, any> = {
            // object-like things
            interface: kw('class'),
            implements: C,
            namespace: C,
  ...
  (CodeMirror as any).registerHelper('wordChars', 'javascript', /[\w$]/);

  (CodeMirror as any).defineMIME('text/javascript', 'javascript');
  (CodeMirror as any).defineMIME('text/ecmascript', 'javascript');
  (CodeMirror as any).defineMIME('application/javascript', 'javascript');
  (CodeMirror as any).defineMIME('application/x-javascript', 'javascript');
  (CodeMirror as any).defineMIME('application/ecmascript', 'javascript');
  (CodeMirror as any).defineMIME('application/json', {
    name: 'javascript',
    json: true
  });
  (CodeMirror as any).defineMIME('application/x-json', {
    name: 'javascript',
    json: true
  });
  (CodeMirror as any).defineMIME('application/ld+json', {
    name: 'javascript',
    jsonld: true
  });
  (CodeMirror as any).defineMIME('text/typescript', {
    name: 'javascript',
    typescript: true
  });
  (CodeMirror as any).defineMIME('application/typescript', {
    name: 'javascript',
    typescript: true
  });
});
