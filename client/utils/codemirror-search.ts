...
function persistentDialog(cm: Editor,
  text: string,
  deflt: string,
  onEnter: (query: string, event: KeyboardEvent) => void,
  replaceOpened: boolean,
  onKeyDown: (event: KeyboardEvent, query: string) => void) {
...
function parseQuery(query: string, state: SearchStateType): string | RegExp {
...
function startSearch(cm: Editor, state: SearchStateType, query: string) {
  var searchDialog = document.querySelector('.CodeMirror-dialog');
  if (searchDialog) {
...
function findNext(
  cm: Editor,
  rev?: boolean,
  callback?: (from: Position, to: Position) => void) {
  cm.operation(function () {
    var state = getSearchState(cm);
...
function clearSearch(cm:Editor) {
  cm.operation(function () {
    var state = getSearchState(cm);
    state.replaceStarted = false;
...
function replaceAll(cm: Editor, query: string | RegExp, text: string) {
  cm.operation(function () {
...
var getQueryDialog = function (): string {
  return `
